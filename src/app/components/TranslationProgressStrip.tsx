...
import { Button, ConfigProvider, Progress, Tooltip, Typography, theme } from "antd";
...
interface TranslationProgressStripProps {
  ...
  /**
   * 批量导出改为「攒齐再打包」:每个文件/语种不再各自触发一次浏览器保存
   * 对话框，而是先进内存队列，全部完成（或被取消）后才允许打成一个 ZIP
   * 下载 —— 按钮常驻在取消按钮旁边，但在跑动时禁用，不会自动触发下载。
   */
  onDownloadZip?: () => void;
  /** ZIP 是否已可下载（本轮至少有一个文件入队，且本轮已经跑完/停止） */
    ...
    <div
      ...
      <div className="flex items-start justify-between" style={{ marginTop: doneWithFailures ? 0 : 10, gap: 12 }}>
        ...
        {/* 取消【不是】危险动作 —— 已译内容全在缓存里,再点一次就续上。用 danger
            ...
            ZIP 下载按钮常驻在取消/关闭按钮旁边:跑动时禁用(队列还没攒齐、
            也不能在半途打包出一个不完整的 ZIP),跑完/取消后一旦队列非空才
            点亮 —— 用户手动点击才触发下载，不是翻译一结束就自动弹保存框。*/}
