...
const SubtitleTranslator = () => {
  ...
  // 批量导出队列:多文件/多语言场景下,每个产物不再各自触发一次浏览器保存
  // 对话框(那样 N 个文件 = N 次系统弹窗,体验很差),而是先攒进这个 ref,
  // 本轮跑完(或被取消)后打成一个 ZIP,由用户手动点「下载」按钮触发 ——
  // 不在翻译一结束就自动下载。单文件单语言场景不入队,维持原有的
  // 「预览 + Export 按钮手动导出单个文件」路径不变。
  const zipQueueRef = useRef<{ name: string; content: string }[]>([]);
  const [zipReady, setZipReady] = useState(false);
  // 队列长度单独镜像成 state:zipQueueRef 是给下载逻辑用的可变累积区,
  // 不能在 render 里直接读 ref.current(React 会警告/报错);按钮上的计数
  // 徽标走这份 state 副本，随每次入队同步更新。
  const [zipFileCount, setZipFileCount] = useState(0);
  const queueZipFile = (content: string, fileName: string) => {
    zipQueueRef.current.push({ name: fileName, content });
    setZipFileCount(zipQueueRef.current.length);
  };
  const resetZipQueue = () => {
    zipQueueRef.current = [];
    setZipReady(false);
  ...
  const handleDownloadZip = async () => {
    ...
    const zip = new JSZip();
    // 同名兜底:理论上 generateFileName 已经用 {lang} 去重,这里只是防御,
    // 避免极端自定义命名模式撞名时后一个文件在 zip 内静默覆盖前一个。
    const usedNames = new Map<string, number>();
    for (const { name, content } of zipQueueRef.current) {
      const count = usedNames.get(name) ?? 0;
      usedNames.set(name, count + 1);
      let entryName = name;
      if (count > 0) {
        const dot = name.lastIndexOf(".");
        entryName = dot > 0 ? `${name.slice(0, dot)} (${count})${name.slice(dot)}` : `${name} (${count})`;
      }
      zip.file(entryName, content);
    }
    const blob = await zip.generateAsync({ type: "blob" });
    const baseName = (multipleFiles[0]?.name || sourceOptions.find((o) => o.value === sourceLanguage)?.label || "subtitles").replace(/\.[^./]+$/, "");
  ...
  const performTranslation = async (sourceText: string, fileNameSet?: string, fileIndex?: number, totalFiles?: number) => {
    ...
    for (const [langPos, currentTargetLang] of targetLangs.entries()) {
      ...
      try {
        ...
        // 多文件 或 多语言 = 批量场景:产物入队打包 ZIP,而不是各自立刻下载
        // (立刻下载会在多文件/多语言时连续弹出多个浏览器保存对话框)。
        const isBatchRun = multiLanguageMode || multipleFiles.length > 1;
        ...
        if (exportMode === "both") {
          ...
          if (isBatchRun) {
            queueZipFile(translatedOnlySubtitle, translatedOnlyFileName);
            queueZipFile(bilingualSubtitle, bilingualFileName);
          } else {
          ...
          // Always export in multi-language/multi-file mode — queued into the
          // ZIP instead of downloading immediately.
          if (isBatchRun) {
            queueZipFile(finalSubtitle, downloadFileName);
          }
          ...
          <Card
            ...
            <Flex gap="small" wrap className="mt-auto pt-4">
              <Button
                ...
                className="flex-1"
                onClick={async () => {
                  // 新一轮开始:清掉上一轮攒下的 ZIP 队列,别让旧文件混进新的
                  // 下载包里(比如换了源文件后忘了下载上一轮的 ZIP)。
                  resetZipQueue();
                  if (uploadMode === "single") {
                    await runTranslation(performTranslation, sourceText, contextAware ? "subtitle" : undefined);
                  } else {
                    await handleMultipleTranslate();
                  }
                  // 本轮(不论正常跑完还是被取消)只要队列里攒了东西就点亮下载
                  // 按钮 —— 不自动下载,交给用户手动点。
                  if (zipQueueRef.current.length > 0) setZipReady(true);
                }}
                disabled={isTranslating}
